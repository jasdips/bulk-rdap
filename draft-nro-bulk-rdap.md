...
* "rdapConformance" -- an array of strings representing a union of all the RDAP extension identifiers used in the
  construction of the bulk data response; note that this is a loose interpretation of ([@!RFC9083, section 4.1])
* "versionId" -- a version 4 Universally Unique IDentifier (UUID, [@!RFC9562, section 5.4]) string that MUST be the same
  for bulk data that was generated by the producer at a given time, such that if multiple bulk files with the same
...
* "productionDate" -- a string containing the date and time in Zulu (Z) format with UTC offset of 00:00 ([@!RFC3339]),
  indicating the time at which the dump/snapshot was taken from the database, even if empty; for example,
  "2024-05-23T19:30:32Z"
* "objectCount" -- a number greater than or equal to zero, representing the count of data objects following the
  metadata object, to help clients detect a partial response for a bulk data request; zero count is possible when an
  RDAP object class is supported but no data exists
...
* An "rdapConformance" member ([@!RFC9083, section 4.1]) MUST be included to indicate the RDAP extensions used for
  constructing the object. Note that this is a loose interpretation of ([@!RFC9083, section 4.1]).
...
The following is an elided example of a bulk data response from an RIR, returning all the IP network objects and
optionally including their relation search links ([@?I-D.ietf-regext-rdap-rir-search]) (note that the ASCII Record
Separator character (0x1E) for each JSON object is not shown):
...
The following is an elided example of a bulk data response from an RIR, returning data for all the RDAP object classes
it supports (note that the ASCII Record Separator character (0x1E) for each JSON object is not shown):
...
When a bulk data response originates from an RIR, the "nroBulkRdap1" extension identifier for this specification (see
...
If both the Bulk RDAP and NRO RDAP Profile extensions are used when constructing a bulk data response, the Bulk RDAP
extension requirements MUST take precedence over the NRO RDAP Profile requirements for data objects; primarily to afford
bulk data compactness.
...
The Content-Type header ([@!RFC9110, section 8.3]) in a bulk data response MUST be set to one of the following:
...
header ([@!RFC9110, section 8.4]) to "gzip" in order to reduce the amount of data that is transmitted.
...
by the RPKI Repository Delta Protocol (RRDP, [@RFC8182]), and the "producer" and "productionDate" fields by the
[NRO Transfer Log Format](https://github.com/nro-ecg/transfer_log/blob/master/transfer_log.jcr).
...
* Should we add an "objectClasses" array of strings in metadata to inform clients about which RDAP object classes the
  bulk data is being returned for? This may be redundant for a bulk data request by an object class but when a server
  returns data for all the objects classes it supports, this could be handy in pre-emptively informing clients about the
  data to follow.
